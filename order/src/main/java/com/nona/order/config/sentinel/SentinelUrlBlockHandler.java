package com.nona.order.config.sentinel;
...
public class SentinelUrlBlockHandler implements BlockExceptionHandler {
    ...
    public void handle(HttpServletRequest request, HttpServletResponse response, BlockException e) throws Exception {
        if (e instanceof FlowException) {
            log.warn("Order服务触发了流控");
            warrperResponse(response, "Order服务触发了流控");
        } else if (e instanceof ParamFlowException) {
            log.warn("Order服务触发了参数流控");
            warrperResponse(response, "Order服务触发了参数流控");
        } else if (e instanceof AuthorityException) {
            log.warn("Order服务触发了授权规则");
            warrperResponse(response, "Order服务触发了授权规则");
        } else if (e instanceof SystemBlockException) {
            log.warn("Order服务触发了系统规则");
            warrperResponse(response, "Order服务触发了系统规则");
        } else {
            log.warn("Order服务触发了降级规则");
            warrperResponse(response, "Order服务触发了降级规则");
        }
    }
